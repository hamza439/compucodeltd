      ...
      <div className="min-h-screen bg-white" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="lg:hidden sticky top-[64px] z-40 bg-white border-b border-gray-200 px-4 py-3">
          <button onClick={() => setTocOpen((o) => !o)} className="flex items-center justify-between w-full text-sm font-semibold text-gray-700" style={{ fontFamily: "'Sora', sans-serif" }}>
            ...
            <span className="text-[#752921] text-lg leading-none transition-transform duration-200" style={{ transform: tocOpen ? "rotate(180deg)" : "rotate(0deg)" }}>▾</span>
          </button>
          {tocOpen && (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-14 flex gap-8 lg:gap-10">
          ...
          <aside className="hidden lg:block w-60 xl:w-64 shrink-0">
            <div className="sticky top-24">
              ...
              <nav className="flex flex-col gap-1">
                {tocSections.map((s) => (
                  <button key={s.id} onClick={() => scrollTo(s.id)}
                    className={`text-left text-sm px-3 py-2 rounded-md transition-all ${s.label.startsWith("↳") ? "pl-6" : ""} ${activeSection === s.id ? "bg-red-50 text-[#752921] font-semibold border-l-2 border-[#752921]" : "text-gray-600 hover:text-[#752921] hover:bg-gray-50"}`}>
                    {s.label}
                  </button>
                ))}
              </nav>
          ...
          <main className="flex-1 min-w-0 space-y-12 sm:space-y-14">
            ...
            <PageSection id="manage" title="Manage Cookie Preferences">
              ...
              <button className="mt-4 inline-flex items-center gap-2 bg-[#752921] hover:bg-[#752921] text-white text-sm font-medium px-5 py-3 rounded-lg transition-colors">
              ...
              <ul className="mt-4 space-y-2">
                {[
                  ...
                  <li key={name} className="flex items-start gap-2 text-sm">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#752921] shrink-0 mt-1.5" />
                    <span className="text-gray-700 font-medium w-32 shrink-0">{name}:</span>
                    <a href={url} target="_blank" rel="noreferrer" className="text-[#752921] underline hover:text-red-800 break-all text-xs">{url}</a>
                  </li>
                ))}
              </ul>
            ...
            <div className="border-t border-gray-200 pt-8 text-xs text-gray-400 text-center">
              Cookie Policy Generated By <span className="text-[#752921] font-semibold">CookieYes</span> Cookie Policy Generator. © Compucode LTD. All rights reserved.
            </div>
          </main>
        </div>
      </div>
